...
This generates metatype XML that management tools (e.g. Karaf's `config:meta` command) can use to display configuration options with names, descriptions, and valid values.
...
- **Karaf distribution** ships `org.eclipse.osgi.technology.incubator.opentelemetry.runtime.otlp.http.cfg` in `etc/`
- **Feature resources** include example `.cfg` files for logging, OTLP/HTTP, and OTLP/gRPC exporters
- **Docker** uses the distribution's OTLP/HTTP config with env var overrides (`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`)
...
- Example `.cfg` files in `src/main/resources/` for logging, OTLP/HTTP, and OTLP/gRPC exporters
- Each exporter type has its own PID: `...runtime.logging`, `...runtime.otlp.http`, `...runtime.otlp.grpc`
...
The runtime automatically sets `service.instance.id` to the OSGi framework UUID (`org.osgi.framework.uuid`).
This is the standard OpenTelemetry semantic convention for distinguishing instances of the same service.

### Resource Attribute Mapping in Prometheus

OpenTelemetry resource attributes map to Prometheus labels via the remote write protocol:

| OTel Resource Attribute | Prometheus Label | Example |
|---|---|---|
| `service.name` | `job` | `osgi-demo-1` |
| `service.instance.id` | `instance` | `53b7ff9c-769a-4f39-a763-c000d31ab41e` |
| `osgi.framework.uuid` | `osgi_framework_uuid` | `53b7ff9c-769a-4f39-a763-c000d31ab41e` |
| `osgi.framework.vendor` | `osgi_framework_vendor` | `Eclipse` |
| other dotted keys | underscored keys | (dots → underscores) |

Dashboard queries use `job` for instance filtering (e.g., `osgi_bundle_count{job=~"$instance"}`).

### Docker Multi-Instance Setup

The `docker-compose.yml` runs two Karaf instances:
- `osgi-app` — `OTEL_SERVICE_NAME=osgi-demo-1`, port 8181
- `osgi-app-2` — `OTEL_SERVICE_NAME=osgi-demo-2`, port 8182

Both report to the same OTel Collector via OTLP/HTTP.
The Multi-Instance Comparison dashboard (`multi-instance.json`) provides side-by-side views.

### Tempo Trace Service Names

In Tempo's span metrics (generated by the metrics generator), `service.name` maps to the `service` label (not `job`).
Dashboard queries targeting `traces_spanmetrics_calls_total` must use `service=~"$instance"` instead of `job=~"$instance"`.
...
| `grafana-multi-instance-overview.png` | 🏷️ Multi-Instance Overview (instance mapping, framework comparison) |
| `grafana-multi-instance.png` | 🏷️ Multi-Instance Full Dashboard (all comparison sections) |
