...
The Docker demo includes pre-configured general checks:
- CPU usage, memory, thread usage, disk space
- Bundles started check
- Framework start check

These require `.cfg` files to activate (`configurationPolicy=REQUIRE`), which are included in the Karaf distribution.
