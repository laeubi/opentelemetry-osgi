...
The integration references the `HttpServiceRuntime` service (provided by [Pax Web](https://ops4j1.jira.com/wiki/spaces/paxweb/overview) in Apache Karaf) and queries its `RuntimeDTO` on every metric collection cycle.
...
The **OSGi HTTP Whiteboard** dashboard shows an overview row with stat panels and repeating per-context rows:

![HTTP Whiteboard Dashboard](../../doc/images/grafana-http-whiteboard.png)

The `context_name` template variable drives repeating rows, showing servlets, filters, listeners, resources, and error pages for each servlet context individually.

## Compatibility

This integration uses the pre-Jakarta HTTP Whiteboard API (`org.osgi.service.http.runtime`) for compatibility with:
- Apache Karaf 4.4.x with Pax Web 8.x
